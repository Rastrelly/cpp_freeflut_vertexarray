...
float func(int a, int b, int c, float x, float y)
{
	return a * sin(b*x)+c*cos(a*y);
}
...
void fillfunc()
{
	pts.clear();
	int sz = w * h;
	float ixc = 0; float iyc = 0; sdtp cp;
	for (int i = 0; i < sz; i++)
	{
		cp.x = (float)ixc;
		cp.y = (float)iyc;
		cp.z = func(1, 2, 1, cp.x, cp.y);
		pts.push_back(cp);
		ixc++;
		if (ixc >= w) { iyc++; ixc = 0; }
	}
...
void makearrays()
{
	pvertices1 = new GLfloat[(w-1)*(h-1)*18-2];
	pcolors1 = new GLfloat[(w - 1)*(h - 1) * 18-2];
	pnormals1 = new GLfloat[(w - 1)*(h - 1) * 18-2];
	int sz = pts.size();
	int k = 0;
	int cnk = 18;
	int cnc = 0;
	...
	printf("entering cycle with %d ops\n", (h-2)*(w-2)*9);
	for (int i = 0; i < h - 1; i++)
	{
		for (int j = 0; j < w - 1; j++)
		{
			...
			{
				//normal
				pvertices1[cn] = pts[k].x;
				pvertices1[cn + 1] = pts[k].y;
				pvertices1[cn + 2] = pts[k].z;
				pcolors1[cn] = 1;
				pcolors1[cn + 1] = 0;
				pcolors1[cn + 2] = 0;
				pnormals1[cn] = 0;
				pnormals1[cn + 1] = 0;
				pnormals1[cn + 2] = 1;
				printf("%d - %d) %f; %f; %f;\n", cn, cn + 2, pvertices1[cn], pvertices1[cn + 1], pvertices1[cn + 2]);
				cn += 3;

				pvertices1[cn] = pts[k + 1].x;
				pvertices1[cn + 1] = pts[k + 1].y;
				pvertices1[cn + 2] = pts[k + 1].z;
				pcolors1[cn] = 0;
				pcolors1[cn + 1] = 1;
				pcolors1[cn + 2] = 0;
				pnormals1[cn] = 0;
				...
				pvertices1[cn] = pts[k + w].x;
				pvertices1[cn + 1] = pts[k + w].y;
				pvertices1[cn + 2] = pts[k + w].z;
				pcolors1[cn] = 0;
				pcolors1[cn + 1] = 0;
				pcolors1[cn + 2] = 1;
				pnormals1[cn] = 0;
				...
				pvertices1[cn] = pts[k + 1 + w].x;
				pvertices1[cn + 1] = pts[k + 1 + w].y;
				pvertices1[cn + 2] = pts[k + 1 + w].z;
				pcolors1[cn] = 0;
				pcolors1[cn + 1] = 1;
				pcolors1[cn + 2] = 1;
				pnormals1[cn] = 0;
		...
		psize = cn - 3 + 1;
		printf("size=%d; requested szie=%d",psize, (w - 1)*(h - 1) * 18 -2);
...
void drawplot()
{
	...
	glEnableClientState(GL_COLOR_ARRAY);
	glEnableClientState(GL_NORMAL_ARRAY);
	glNormalPointer(GL_FLOAT, 0, pnormals1);
	glColorPointer(3, GL_FLOAT, 0, pcolors1);
	glVertexPointer(3, GL_FLOAT, 0, pvertices1);

	glPushMatrix();
	//glTranslatef(-(w/2), 0, -(h/2));  // move to upper-right corner
	glRotatef(90, 0, 0, 1);

	glDrawArrays(GL_TRIANGLES, 0, (w-1)*(h-1)*6);
